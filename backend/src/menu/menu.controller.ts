...
class MenuItemDto { @IsUUID() categoryId!: string; @IsString() name!: string; @IsOptional() @IsString() description?: string; @IsNumber() @Min(0) price!: number; @IsOptional() @IsString() imageUrl?: string; }
...
export class MenuController {
  ...
  @Get('menu-items') items(@Req() request: Request, @Query('categoryId') categoryId?: string, @Query('activeOnly') activeOnly?: string) {
    return this.prisma.menuItem.findMany({ where: { tenantId: request.tenantId!, ...(categoryId ? { categoryId } : {}), ...(activeOnly === 'true' ? { isActive: true } : {}) }, include: { category: true }, orderBy: { name: 'asc' } });
  }
