...
import { toReceiptData } from "@/lib/printing/types";
...
export function ReceiptPrint({ receiptId }: { receiptId: string }) {
  ...
  useEffect(() => {
    if (!order || !printerConfig) return;
    const onAfterPrint = () => {
      if (!browserPrintPending.current) return;
  ...
  }, [order, printerConfig, queryClient, receiptId]);
  ...
  return (
    <main className="receipt-page" style={{ "--receipt-paper-width": `${printerConfig?.paperWidth ?? 80}mm` } as CSSProperties}>
      ...
      <article className={`receipt-paper receipt-paper-${printerConfig?.paperWidth ?? 80}`}>
        ...
        <section aria-label="Bill information" className="receipt-meta">
          <div><span>Bill:</span><strong>{order.billNumber}</strong></div>
        ...
        <section aria-label="Payment summary" className="receipt-payment-summary">
          ...
          <div><span>Amount paid</span><span>NPR {paid.toLocaleString()}</span></div>
