...
export function TableManagement() {
  ...
  return (
    <div className="table-management">
      ...
      <section aria-labelledby="configured-tables-heading" className="card table-list-card"><div className="table-list-heading"><h2 id="configured-tables-heading">Configured tables</h2><span className="tag">{tables.length} total</span></div>{query.error && <p className="error" role="alert">Unable to load tables.</p>}{deleteMutation.error && <p className="error" role="alert">{deleteMutation.error instanceof Error ? deleteMutation.error.message : "Unable to delete table."}</p>}{tables.length ? <ul className="configured-table-list">{tables.map((table) => <li className="configured-table-row" key={table.id}><span>{table.tableNumber}</span><button aria-label={`Delete ${table.tableNumber}`} className="table-delete-button" disabled={deleteMutation.isPending || Boolean(table.orders?.length)} onClick={() => { if (window.confirm(`Delete ${table.tableNumber}?`)) deleteMutation.mutate(table.id); }} title={table.orders?.length ? "Close the table's open bill before deleting it" : `Delete ${table.tableNumber}`} type="button"><Trash2 aria-hidden="true" size={16} /><span>{deleteMutation.isPending && deleteMutation.variables === table.id ? "Deleting…" : "Delete"}</span></button></li>)}</ul> : <p className="muted">No tables configured yet.</p>}</section>
    </div>
  );
}
