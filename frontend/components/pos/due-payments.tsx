...
export function DuePayments() {
  ...
  const mutation = useMutation({ mutationFn: ({ id, amount }: { id: string; amount: number }) => api.createPayment(id, { method: "CASH", amount }), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["orders"] }); setActiveId(null); setAmount(""); } });
